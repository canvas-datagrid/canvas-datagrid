...
export default function (self) {
  ...
  self.setFilter = function (column, value) {
    console.warn(
      'setFilter is deprecated and will be removed in a future release' +
        'in favor of implementing your own filter function using registerFilter',
    );
  ...
  /**
   * Registers a filter function to be applied for each row. This takes precedence over any legacy filters
   * that may applied programmatically through 'setFilter' or by the user through the context menu.
   * @param {function} filterFn Function that is called for each row, and returns a boolean whether it should be showed (true) or not (false).
   */
  self.registerFilter = function (filterFn) {
    self.filterFunction = filterFn;
  };

  /**
   * Unregisters any filter function, if set. If no filter function is registered, no filtering will
   * be applied.
   * @param {function} filterFn Function that is called for each row, and returns a boolean whether it should be showed (true) or not (false).
   */
  self.unregisterFilter = function (filterFn) {
