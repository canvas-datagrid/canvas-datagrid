...
export default function (self, ctor) {
  ...
  self.hasActiveFilters = function () {
    return (
      (self.columnFilters && Object.keys(self.columnFilters).length > 0) ||
      !!self.filterFunction
    );
  };
  ...
  self.getFilteredAndSortedViewData = function (originalData) {
    ...
    if (self.filterFunction) {
      newViewData = newViewData.filter(([row, originalRowIndex]) =>
        self.filterFunction({
      ...
      // These are legacy filters, set by the context menu, or through setFilter,
      // and are limited to just matching on text/regexes. Providing a custom filter
      // function is more flexible, and will be the preferred way going forward.
  ...
  self.init = function () {
    ...
    self.intf.registerFilter = self.registerFilter;
    self.intf.unregisterFilter = self.unregisterFilter;
